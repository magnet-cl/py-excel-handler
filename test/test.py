...
class ForeignKeyExcelHandler(ExcelHandler):
    first = fields.ForeignKeyField(model=Model, col=0, default=None)
...
class TestChangeSheetCase(unittest.TestCase):
    def test_change_sheet_by_index(self):
        eh = MyExcelHandler(path="test/test.xlsx", mode="r")
        eh.set_sheet(0)
...
class TestExcelHandlerCase(unittest.TestCase):
    def test_read_rows(self):
        ...
        second_row = eh.read_rows(column_structure, starting_row=2)
...
class TestErrorHandler(unittest.TestCase):
    ...
    def test_read(self):
        eh = self.excel_handler_cls(path="test/test.xlsx", mode="r")
        data = eh.read()
...
class TestCustomExcelHandler(unittest.TestCase):
    ...
    def test_write(self):
        ...
        choices = dict(self.excel_handler_cls.CHOICES)

        self.assertEqual(
            eh.sheet.cell(column=2, row=2).value, choices[data[0]["second"]]
        )

        self.assertEqual(
            eh.sheet.cell(column=2, row=3).value, choices[data[1]["second"]]
        )
